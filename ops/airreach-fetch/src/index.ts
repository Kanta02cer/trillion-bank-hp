/**
 * AirReach first-party fetch proxy.
 * Fetches public HTML / text for readiness diagnosis with SSRF guards.
 * Does not measure live AI citation rates.
 */

const MAX_BYTES = 900_000;
const MAX_REDIRECTS = 3;
const FETCH_TIMEOUT_MS = 12_000;
const USER_AGENT = 'TrillionBank-AirReach/1.0 (+https://trillion-bank.jp/airreach/; readiness-check)';
...
function handleOptions(request: Request, corsOrigin: string | null): Response {
  if (!corsOrigin) {
...
function corsHeaders(corsOrigin: string | null, extra: Record<string, string> = {}): Headers {
...
function jsonError(status: number, message: string, corsOrigin: string | null): Response {
...
function sanitizeTargetUrl(input: string): URL {
...
async function fetchPublicDocument(start: URL): Promise<{
  status: number;
  body: string;
  contentType: string;
  finalUrl: string;
  ...
  for (let hop = 0; hop <= MAX_REDIRECTS; hop += 1) {
    ...
    if (
      contentType &&
      !/^(text\/html|application\/xhtml\+xml|text\/plain|text\/markdown|application\/json)/i.test(contentType)
    ) {
